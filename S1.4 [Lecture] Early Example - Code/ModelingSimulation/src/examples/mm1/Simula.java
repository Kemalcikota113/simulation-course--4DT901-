...
public class Simula {
	...
	//If you want to have a different simulation for each run, uncomment the following line
	...
	private double total_time_in_system;
	

	// private double area_under_Lt = area_under_Qt + area_under_Bt;
	...
	private XYChart getChartQt() {
		...
		XYChart chart = new XYChartBuilder().width(800).height(600).title("Average Number in Queue").xAxisTitle("t").yAxisTitle("Q(t)").build();
	...
	private XYChart getChartBt() {
		...
		XYChart chart = new XYChartBuilder().width(800).height(600).title("Server Utilization").xAxisTitle("t").yAxisTitle("B(t)").build();
	...
	public void report(String s) {
		...
		this.log("Average number in queue " + area_under_Qt / this.clock + "\n\n");
		this.log("Server utilization " + area_under_Bt / clock + "\n\n");

		// task 7
		this.log("Time average in the system " + (area_under_Bt + area_under_Qt) / clock + "\n\n");
		this.log("Average total time in the system " + (total_time_in_system / num_delayed) + "minutes \n\n");
	...
	private void depart() {
		...
		if (this.num_in_q == 0) {
			...
			double service_time = ed.random(mean_service);   // Random service time generated
			//double service_time = ThreadLocalRandom.current().nextDouble(0.5, 1.5);

			double total_time = delay + service_time;        // Total time in system
			this.total_time_in_system += total_time;
	...
	public static void main(String[] args) {
		...
		/* Read input parameters. */
		double mean_interarrival = 0.8;
		//double mean_service = Double.parseDouble(args[1]);
		double mean_service = 1;
		int num_delays_required = 580;
